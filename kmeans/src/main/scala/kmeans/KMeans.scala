...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val mappedPoints = points.groupBy(findClosest(_, means))
    means.map(mean => mean -> mappedPoints.getOrElse(mean, GenSeq())).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    if (oldMeans.isEmpty && newMeans.isEmpty) true
      ...
      if (oldMeans.head.squareDistance(newMeans.head) > eta) false
      else converged(eta)(oldMeans.tail, newMeans.tail)
  }
