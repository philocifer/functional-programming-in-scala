...
object FunSets {
   ...
   */
    def diff(s: Set, t: Set): Set = elem => (contains(s, elem) && !contains(t, elem)) || (!contains(s, elem) && contains(t, elem))
   ...
   */
    def map(s: Set, f: Int => Int): Set = elem => exists(s, (mapElem: Int) => f(mapElem) == elem)
