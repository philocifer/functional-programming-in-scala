...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val columns = columnNames.tail.map(column => StructField(column, DoubleType, nullable = false))
    StructType(firstColumn :: columns)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def classifyColumnsRec(columnNames: List[String], primary: List[Column], working: List[Column], leisure: List[Column]): (List[Column], List[Column], List[Column]) =
      ...
      classifyColumnsRec(columnNames, Nil, Nil, Nil)
  }
  ...
  def timeUsageSummary(
    ...
    def sumHours(columns: List[Column], name: String): Column = columns.aggregate[Column](lit(0.0d))((sum, column) => sum + (column / 60.0), (column1, column2) => column1 + column2).as(name)
    val primaryNeedsProjection: Column = sumHours(primaryNeedsColumns, "primaryNeeds")
    val workProjection: Column = sumHours(workColumns, "work")
    val otherProjection: Column = sumHours(otherColumns, "other")
    df
